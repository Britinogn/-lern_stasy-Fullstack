    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-400 via-white pt-20 to-blue-500">
        ...
        <div v-else class="flex items-center justify-center min-h-screen">
            <div class="text-center">
                ...
                <h3 class="text-xl font-semibold text-slate-900 mb-2">Error</h3>
                <p class="text-slate-600 mb-6">{{ error || 'Course not found or you don\'t have permission to edit it.' }}</p>
                <router-link 
                    to="/courses" 
    ...
    methods: {
        async loadCourse() {
            ...
            try {
                ...
                this.isPublished = res.data.isPublished || false;
                ...
                if (!instructorId || !userId || instructorId.toString() !== userId.toString()) {
                    this.error = 'You are not authorized to edit this course';
                    // Keep error visible instead of immediate redirect
                    return;
                }
                
            } catch (err) {
                console.error('Error fetching course:', err);
                this.error = err.response?.data?.message || 'Failed to load course';
            } finally {
                this.loading = false;
            }
        },
        ...
        async fetchProfileIfNeeded() {
            if (!auth.user && auth.token) {
                try {
                    ...
                    console.log('Fetched user:', auth.user);
                } catch (e) {
                    console.error('Failed to fetch profile:', e);
                    this.error = 'Unable to load user profile. Please log in again.';
                    setTimeout(() => {
                ...
                this.currentUserId = auth.user.id;
                console.log('Using existing user:', auth.user);
            } else {
